...
const TESTS = {
  ...
  // this exact set (flagship example, basic auth, -k/insecure, each in
  // every language) already caught real bugs during development.
  'curl-converter': async (p) => {
    ...
    const langChecks = {
      csharp: ['HttpRequestMessage(HttpMethod.Post', 'TryAddWithoutValidation("Authorization"', 'StringContent(', 'application/json'],
      python: ['import requests', 'requests.post(url', 'headers=headers', '"Authorization": "Bearer'],
      fetch: ['await fetch(', 'method: "POST"', 'headers,', 'body: "{'],
      axios: ["require('axios')", 'axios.post(', '"Authorization"'],
      powershell: ['Invoke-RestMethod', '-Method Post', '$headers', 'Authorization" = "Bearer'],
      go: ['package main', 'http.NewRequest("POST"', 'req.Header.Set("Authorization"', 'strings.NewReader'],
      java: ['HttpClient.newHttpClient()', '.method("POST"', '.header("Authorization"', 'BodyPublishers.ofString'],
    };
    const authTest = {
      csharp: ['AuthenticationHeaderValue("Basic"', 'admin:hunter2'],
      python: ['auth=("admin", "hunter2")'],
      fetch: ['btoa("admin:hunter2")'],
      axios: ['auth: { username: "admin", password: "hunter2" }'],
      powershell: ['[Convert]::ToBase64String', 'admin:hunter2'],
      go: ['req.SetBasicAuth("admin", "hunter2")'],
      java: ['Base64.getEncoder().encodeToString("admin:hunter2"'],
    };
    const insecureTest = {
      csharp: ['DangerousAcceptAnyServerCertificateValidator'],
      python: ['verify=False'],
      fetch: ['NODE_TLS_REJECT_UNAUTHORIZED'],
    ...
    const failures = [];
    const checkLang = async (label, command, table) => {
      await p.fill('#curlInput', command);
      await p.click('#btnCurlConvert');
      await p.waitForTimeout(250);
      for (const [lang, fragments] of Object.entries(table)) {
        await p.selectOption('#langSelect', lang);
        await p.waitForTimeout(80);
        const out = await p.textContent('#codeOutput');
        const missing = fragments.filter(f => !out.includes(f));
        if (missing.length) failures.push(`${label}/${lang}: missing ${missing.join(', ')}`);
      }
    };

    await checkLang('flagship', EXAMPLE, langChecks);
    await checkLang('basic-auth', 'curl -u admin:hunter2 https://api.example.com/secure', authTest);
    await checkLang('insecure', 'curl -k https://self-signed.example.com/', insecureTest);

    // Switching languages reuses the last parsed command rather than
    // re-parsing -- verify that cache actually gets used, not just that
    // each language works when freshly converted.
    await p.fill('#curlInput', 'curl -X DELETE https://api.example.com/items/9');
    await p.click('#btnCurlConvert'); await p.waitForTimeout(200);
    await p.selectOption('#langSelect', 'go'); await p.waitForTimeout(100);
    const goOut = await p.textContent('#codeOutput');
    if (!goOut.includes('http.NewRequest("DELETE"')) failures.push('language switch without re-conversion did not update output');

    // -G moves -d data into the URL as a query string and forces GET,
    // instead of sending a body -- a distinct code path from the default.
    await p.fill('#curlInput', `curl -G 'https://api.example.com/search' -d 'q=test' -d 'page=2'`);
    await p.click('#btnCurlConvert'); await p.waitForTimeout(200);
    await p.selectOption('#langSelect', 'python'); await p.waitForTimeout(100);
    const pyGet = await p.textContent('#codeOutput');
    if (!pyGet.includes('q=test&page=2') || pyGet.includes('data =')) failures.push('-G query-string mode did not move data into the URL');

    return { ok: failures.length === 0, detail: failures.length ? failures.join(' | ') : '23/23 checks passed (7 languages x 3 cases + 2 mechanism checks)' };
  },
};
